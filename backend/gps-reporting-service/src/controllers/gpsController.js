...
const hasOpenSOS = async (boatId) => {
  const latest = await BoatLocation.findOne({ boatId }).sort({ timestamp: -1 }).lean();
...
exports.cancelSOS = async (req, res) => {
  try {
    ...
    if (!boatId) {
      return res.status(400).json({ success: false, message: 'BoatId (nationalId) is required' });
    }
    ...
    try {
      ...
      console.error('[gps] police notify failed on cancel:', notifyErr?.message);
    }

    return res.json({ success: true, message: 'SOS canceled', data: cancelEntry });
  } catch (error) {
